// Pull in build-time constants generated by build.rs
...
fn main() {
    ...
    println!("{pakage_name} v.{version} | built: {BUILD_DATE}");
    ...
    let matches = Command::new(pakage_name)
        ...
        // Path simplification
        .arg(Arg::new("simplify-paths")
            .long("simplify-paths").action(ArgAction::SetTrue)
            .help("Simplify path data: absolute coords, expand H/V/S/T, merge collinear L"))
        .arg(Arg::new("combine-paths")
            .long("combine-paths").action(ArgAction::SetTrue)
            .help("Combine consecutive paths with identical attributes (picosvg-style)"))
        // SVG document
    ...
    let opts = ScourOptions {
        ...
        simplify_paths:           matches.get_flag("simplify-paths"),
        combine_paths:            matches.get_flag("combine-paths"),
    ...
    let svg_text = if is_svgz_in {
        ...
        d.read_to_string(&mut s).unwrap_or_else(|e| {
            eprintln!("{pakage_name}: error decompressing SVGZ: {}", e);
            std::process::exit(1);
        });
        s
    } else {
    ...
    if opts.verbose && !opts.quiet {
        ...
        if stats.paths_simplified > 0 {
            eprintln!("{pakage_name}: paths simplified: {}", stats.paths_simplified);
        }
        if stats.paths_combined > 0 {
            eprintln!("{pakage_name}: paths combined: {}", stats.paths_combined);
        }
        if stats.empty_defs_removed > 0 {
            eprintln!("{pakage_name}: empty <defs> removed: {}", stats.empty_defs_removed);
        }
    }
