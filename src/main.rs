...
fn main() {
    let matches = Command::new("scrubr")
        ...
        .arg(Arg::new("output")
            .short('o').long("output").value_name("OUTPUT.SVG")
            .help("Output SVG file (default: stdout)"))
        .arg(Arg::new("quiet")
            .short('q').long("quiet").action(ArgAction::SetTrue)
        ...
        .arg(Arg::new("nindent")
            .long("nindent").value_name("NUM").default_value("1")
            .help("Depth of indentation (default: 1)"))
        .arg(Arg::new("no-line-breaks")
            .long("no-line-breaks").action(ArgAction::SetTrue)
    ...
    let protect_ids_list: Vec<String> = matches
        .get_one::<String>("protect-ids-list")
        .map(|s| s.split(',').map(|x| x.trim().to_string()).collect())
        .unwrap_or_default();

    let indent = match matches.get_one::<String>("indent").unwrap().as_str() {
    ...
    let descriptive = matches.get_flag("remove-descriptive-elements");

    let opts = ScourOptions {
        precision,
        c_precision,
        ...
        renderer_workaround:     !matches.get_flag("no-renderer-workaround"),
        strip_xml_prolog:         matches.get_flag("strip-xml-prolog"),
        remove_titles:            matches.get_flag("remove-titles")   || descriptive,
        remove_descriptions:      matches.get_flag("remove-descriptions") || descriptive,
        remove_metadata:          matches.get_flag("remove-metadata") || descriptive,
    ...
    let svg_text = if is_svgz_in {
        ...
        d.read_to_string(&mut s).unwrap_or_else(|e| {
            eprintln!("scrubr: error decompressing SVGZ: {}", e);
            std::process::exit(1);
        });
        s
    } else {
